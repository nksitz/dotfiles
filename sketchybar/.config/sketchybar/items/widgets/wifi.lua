...
local function update_vpn()
	sbar.exec("wg show interfaces 2>/dev/null", function(wg)
		if wg and wg:match("%S") then
			wifi:set({ icon = { string = "󱚿" } })
			vpn_item:set({
				drawing = true,
				label = { string = "WireGuard" },
			})
		else
			sbar.exec("scutil --nc status ProtonVPN 2>/dev/null", function(output)
				if output and output:match("^Connected") then
					...
					vpn_item:set({
						drawing = true,
						label = { string = "ProtonVPN" },
					})
				else
					vpn_item:set({ drawing = false })
					-- icon will be reset by the wifi_change handler below
				end
			end)
		end
	end)
...
-- Network update (upload/download speeds)
...
-- Wifi connection state + VPN check
